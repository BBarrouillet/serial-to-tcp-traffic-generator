...
class SerialThroughput:
    def __init__(self):
        ...
        if confsection:
            ...
            if val == 'True':
                ...
                self.cts_rts.set(0)
        ttk.Checkbutton(top, text="RTS/CTS Key-Up", variable=self.cts_rts).grid(row=currentrow, column=1, padx=5, pady=5, sticky='W')
        currentrow += 1
        ...
        if confsection:
            ...
            if val == 'True':
                ...
                self.hw_flow_control.set(0)
        ttk.Checkbutton(top, text="Hardware Flow Control (RTS/CTS)", variable=self.hw_flow_control).grid(row=currentrow, column=1, padx=5, pady=5, sticky='W')
        currentrow += 1
        ...
        self.start_stop_button = Button (top, text="Start", justify=CENTER, bd=6,
                                    command = lambda : self.start_stop_click(send.get(), tcp_host.get(), tcp_port.get(), baud.get(), dBits.get(), prty.get(),
                                                            sBits.get(), size_value.get(), response_size_value.get(), float(tx_delay_value.get()),
                                                            cnt_value.get(), mode.get(), read_timeout.get(),
                                                            sending_mode.get(), target_bw_value.get()))
    ...
    def start_stop_click(self, sender_port, tcp_host, tcp_port, baud_rate, data_bits, parity, stop_bits, packet_size, response_size, tx_delay_chars, packet_count, mode, timeout, sending_mode="Pipelined", target_bw=9600):
        if (self.send_thread and self.send_thread.is_alive()) or (self.receive_thread and self.receive_thread.is_alive()):
            ...
            try:
                ...
                sender = serial.Serial (sender_port, baud_rate, rtscts=use_hw_flow_control, stopbits=sb, bytesize=bs, parity=pm, timeout=timeout/1000.0, write_timeout=10)
    ...
    def print_values (self, test_finished):
        ...
        if self.mode == "Throughput":
            ...
            self.result_box.insert(INSERT, "  Valid Packets      : %d / %d\n" % (len(self.received_ids), self.packet_count))
            self.result_box.insert(INSERT, "  Bad Signature      : %d\n" % self.rx_bad_signature_count)
            self.result_box.insert(INSERT, "  Payload Errors     : %d\n" % self.rx_payload_error_count)
            ...
            if test_finished:
                ...
                missing = expected - self.received_ids
                if missing:
                    self.result_box.insert(INSERT, "  Missing Packet IDs : %s\n" % self.format_id_ranges(missing))
                else:
                    self.result_box.insert(INSERT, "  Missing Packet IDs : None\n")
                self.result_box.insert(INSERT, "  Received IDs       : %s\n" % self.format_id_ranges(self.received_ids))
