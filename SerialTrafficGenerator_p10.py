...
class SerialThroughput:
    def __init__(self):
        ...
        top.title("4RF Serial Traffic Generator 1.9")
        ...
        tcp_host = StringVar()
        tcp_host.set("192.168.1.100")
        if confsection:
            tcp_host.set(confsection.get('tcp_host', '192.168.1.100'))
        Entry(top, textvariable=tcp_host).grid(row=currentrow, column=1, padx=5, pady=5, sticky='EW')
        currentrow += 1
        ...
        tcp_port.set(5000)
        if confsection:
            tcp_port.set(int(confsection.get('tcp_port', '5000')))
        Entry(top, textvariable=tcp_port).grid(row=currentrow, column=1, padx=5, pady=5, sticky='EW')
        currentrow += 1

        Label(top, text="Mode").grid(row=currentrow, column=0, padx=5, pady=5, sticky='E')
        mode = StringVar()
        combo = ttk.Combobox(top, textvariable=mode)
        combo['values'] = ["Throughput", "Latency"]
        combo.current(0)
        ...
        self.sending_mode_label = Label(top, text="Sending Mode")
        self.sending_mode_label.grid(row=currentrow, column=0, padx=5, pady=5, sticky='E')
        sending_mode = StringVar()
        combo = ttk.Combobox(top, textvariable=sending_mode)
        combo['values'] = ["Pipelined", "Fixed Bandwidth"]
        combo.current(0)
        combo.bind("<<ComboboxSelected>>", lambda _: self.sending_mode_combo_changed(sending_mode.get()))
        if confsection:
            val = confsection.get('sending_mode', '')
            if val in combo['values']:
                combo.current(combo['values'].index(val))
        self.sending_mode_combo = combo
        combo.grid(row=currentrow, column=1, padx=5, pady=5, sticky='EW')
        currentrow += 1

        self.target_bw_label = Label(top, text="Target Bandwidth\n(bits/s)")
        self.target_bw_label.grid(row=currentrow, column=0, padx=5, pady=5, sticky='E')
        target_bw_value = IntVar()
        target_bw_value.set(9600)
        if confsection:
            val = confsection.get('target_bandwidth', '9600')
            target_bw_value.set(int(val))
        self.target_bw_entry = Entry(top, textvariable=target_bw_value)
        self.target_bw_entry.grid(row=currentrow, column=1, padx=5, pady=5, sticky='W')
        currentrow += 1

        # Hide sending mode options by default (only shown in Throughput mode)
        if not confsection or confsection.get('sending_mode', 'Pipelined') == 'Pipelined':
            self.target_bw_label.grid_remove()
            self.target_bw_entry.grid_remove()
        ...
        if confsection:
            ...
            if val == 'True':
                ...
                self.cts_rts.set(0)
        ttk.Checkbutton(top, text="RTS/CTS Key-Up", variable=self.cts_rts).grid(row=currentrow, column=1, padx=5,
                                                                                pady=5, sticky='W')
        currentrow += 1
        ...
        if confsection:
            ...
            if val == 'True':
                ...
                self.hw_flow_control.set(0)
        ttk.Checkbutton(top, text="Hardware Flow Control (RTS/CTS)", variable=self.hw_flow_control).grid(row=currentrow,
                                                                                                         column=1,
                                                                                                         padx=5, pady=5,
                                                                                                         sticky='W')
        currentrow += 1
        ...
        self.start_stop_button = Button(top, text="Start", justify=CENTER, bd=6,
                                        command=lambda: self.start_stop_click(send.get(), tcp_host.get(),
                                                                              tcp_port.get(), baud.get(), dBits.get(),
                                                                              prty.get(),
                                                                              sBits.get(), size_value.get(),
                                                                              response_size_value.get(),
                                                                              float(tx_delay_value.get()),
    ...
    def mode_combo_changed(self, mode):
        ...
        if mode == "Throughput":
            self.sending_mode_label.grid()
            self.sending_mode_combo.grid()
            self.sending_mode_combo_changed(self.sending_mode_combo.get())
        else:
            self.sending_mode_label.grid_remove()
            self.sending_mode_combo.grid_remove()
            self.target_bw_label.grid_remove()
            self.target_bw_entry.grid_remove()

    def sending_mode_combo_changed(self, sending_mode):
        if sending_mode == "Fixed Bandwidth":
            self.target_bw_label.grid()
            self.target_bw_entry.grid()
        else:
            self.target_bw_label.grid_remove()
            self.target_bw_entry.grid_remove()
    ...
    def start_stop_click(self, sender_port, tcp_host, tcp_port, baud_rate, data_bits, parity, stop_bits, packet_size,
                         response_size, tx_delay_chars, packet_count, mode, timeout, sending_mode="Pipelined",
                         target_bw=9600):
        if (self.send_thread and self.send_thread.is_alive()) or (
                self.receive_thread and self.receive_thread.is_alive()):
            if self.cancel_test():
            ...
            if self.hw_flow_control.get() == 1:
                config['config']['hw_flow_control'] = 'True'
            else:
                config['config']['hw_flow_control'] = 'False'
            ...
            try:
                ...
                # Connect to the Serial port for sending
                use_hw_flow_control = self.hw_flow_control.get() == 1
                sender = serial.Serial(sender_port, baud_rate, rtscts=use_hw_flow_control, stopbits=sb, bytesize=bs,
                                       parity=pm, timeout=timeout / 1000.0, write_timeout=10)

                # Ensure that RTS signal is initially low. This is often used as a key-up signal
                ...
                if mode == "Throughput":
                    ...
                    self.receive_thread = threading.Thread(target=self.read_data_thread, args=(self.tcp_socket,))

                    # Start the Threads
                    self.receive_thread.start()
                    self.send_thread.start()
                elif mode == "Latency":
                    self.send_thread = threading.Thread(target=self.latency_thread, args=(sender, self.tcp_socket))
    ...
    def flow_control_tx_start(self, port: serial.Serial):
        ...
        if self.use_flow_control:
            flow_control_start = time.perf_counter()
            port.setRTS(True)
            ...
            while port.getCTS():
                if time.perf_counter() - flow_control_start > 10:
                    raise Exception('Timeout waiting for flow control')

        self.tx_start = time.perf_counter()
    ...
    def flow_control_tx_finish(self, port, tx_delay_seconds):
        ...
        sleep_duration = tx_delay_seconds - (time.perf_counter() - self.tx_start)
        ...
        if sleep_duration > 0:
            ...
            s = time.perf_counter()
            while time.perf_counter() - s < sleep_duration:
                pass
    ...
    def tcp_recv(self, sock, num_bytes):
        """Read exactly num_bytes from a TCP socket, similar to serial port read().
        Returns fewer bytes if timeout or connection closes."""
        data = bytearray()
    ...
    def send_data_thread(self, send_port):
        ...
        if self.sending_mode == "Fixed Bandwidth" and self.target_bw > 0:
            target_interval = self.packet_size * self.bits_per_char() / self.target_bw
        else:
        ...
        try:
            for x in range(0, self.packet_count):
                ...
                if target_interval:
                    ...
                    # Pipelined: wait for receiver to consume a packet
            ...
            print("Sending failed " + str(err))

    def read_data_thread(self, receive_socket):
        self.rx_byte_count = 0
        self.last_rx_time = None
        buffer = bytearray()
        sig_bytes = struct.pack('>H', HEADER_SIGNATURE)
        packets_parsed = 0
        stream_pos = 0  # absolute position in stream where buffer starts
        last_sig_pos = None  # absolute position of previous signature

        while not self.stop_threads:
            if packets_parsed >= self.packet_count:
                break
            ...
            while len(buffer) >= self.packet_size and packets_parsed < self.packet_count:
                # Scan for the signature in the buffer
                pos = buffer.find(sig_bytes)
                if pos < 0:
                    # No signature found, keep last byte (could be start of signature)
                    stream_pos += len(buffer) - 1
                    buffer = buffer[-1:]
                    break
                if pos > 0:
                    # Discard misaligned bytes before signature
                    stream_pos += pos
                    buffer = buffer[pos:]
                if len(buffer) < self.packet_size:
                    break

                # Measure actual distance between consecutive signatures
                current_sig_pos = stream_pos
                if last_sig_pos is not None:
                    self.rx_packet_sizes.append(current_sig_pos - last_sig_pos)
                last_sig_pos = current_sig_pos
                ...
                if self.sending_mode == "Pipelined" and self.pipeline_max:
    ...
    def print_values(self, test_finished):
        ...
        if self.mode == "Throughput":
            ...
            self.result_box.insert(INSERT,
                                   "  Received OK        : %d / %d\n" % (len(self.received_ok_ids), self.packet_count))
            self.result_box.insert(INSERT, "  Corrupted          : %d\n" % len(self.received_corrupted_ids))
            self.result_box.insert(INSERT, "  Bad Signature      : %d\n" % self.rx_bad_signature_count)
            if self.rx_packet_sizes:
                min_sz = min(self.rx_packet_sizes)
                max_sz = max(self.rx_packet_sizes)
                avg_sz = sum(self.rx_packet_sizes) / len(self.rx_packet_sizes)
                self.result_box.insert(INSERT, "  Rx Packet Size     : min=%d avg=%.1f max=%d (expected %d)\n" % (
                min_sz, avg_sz, max_sz, self.packet_size))
            if test_finished:
                expected = set(range(self.tx_packet_count))
                missing = expected - all_received
                if missing:
                    self.result_box.insert(INSERT, "  Missing IDs        : %s\n" % self.format_id_ranges(missing))
                else:
                    self.result_box.insert(INSERT, "  Missing IDs        : None\n")
                if self.received_corrupted_ids:
                    self.result_box.insert(INSERT, "  Corrupted IDs      : %s\n" % self.format_id_ranges(
                        self.received_corrupted_ids))
                self.result_box.insert(INSERT,
                                       "  OK IDs             : %s\n" % self.format_id_ranges(self.received_ok_ids))
            ...
            self.result_box.insert(INSERT, "Received OK        : %d\n" % len(self.received_ok_ids))
            self.result_box.insert(INSERT, "Corrupted          : %d\n" % len(self.received_corrupted_ids))
            self.result_box.insert(INSERT, "Bad Signature      : %d\n\n" % self.rx_bad_signature_count)
            if test_finished:
                expected = set(range(self.tx_packet_count))
        ...
        if len(self.latencies) > 0:
            ...
            if test_finished:
                ...
                self.result_box.insert(INSERT,
                                       "  Ideal FILO at {:d} bps : {:.3f}\n".format(self.baud_rate, ideal_latency))
                self.result_box.insert(INSERT, "  Extra FILO (2)         : {:.3f}\n".format(added_latency))
                self.result_box.insert(INSERT, "\n")
                self.result_box.insert(INSERT,
                                       "(1) LIFO latency may not be accurate, as it cannot be accurately measured with PC serial ports\n")
                ...
                self.result_box.insert(INSERT,
                                       "This is the time between the first bit into the transmitter and the last bit out of the receiver, or simply the total time taken to transfer the data.\n\n")
                self.result_box.insert(INSERT, "LIFO) Last In First Out.\n")
                self.result_box.insert(INSERT,
                                       "This is the time between the last bit into the transmitter and the first bit out of the receiver. Assuming the device is buffering each packet, this gives a measure of the internal transfer speed of the device.")
